...
from torchvision.transforms import RandomHorizontalFlip, Resize, ToTensor
...
def load_datasets(path, n):
    ...
    test_transform = transforms.Compose([
        transforms.Resize((224,224)), 
        transforms.ToTensor(), 
        transforms.Normalize(ean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    ])
    ...
    train_indexes = random.sample(fake_indexes, n*0.4)+ random.sample(real_indxes, n*0.4)
    ...
    random.shuffle(fake_index_test)
    random.shuffle(real_index_test)
    ...
    test_indexes = fake_indexes_test[: n*0.1]+real_indexes_test[:n*0.1]
    val_indexes = fake_index_test[n*0.1:n*0.125] +real_index_test[n*0.1:n*0.125]
    testdataset = Subset(ftestdataset, test_indexes)
    valdataset = Subset(ftestdataset, val_indexes)
    #validating data
    fake_index_val = 
